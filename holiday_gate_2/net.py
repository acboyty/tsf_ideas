import torch 
import torch.nn as nn


class RNNNet(nn.Module):
    def __init__(self, input_dim, hid_dim, output_dim, seq_len):
        super(RNNNet, self).__init__()

        self.input_dim = input_dim
        self.hid_dim = hid_dim
        self.output_dim = output_dim
        self.seq_len = seq_len

        self._build()

    def _build(self):
        self.rnn = nn.GRU(self.input_dim, self.hid_dim)
        self.lin = nn.Linear(self.hid_dim, self.output_dim)

        self.ar = nn.Linear(self.seq_len, self.output_dim)

    def forward(self, x):
        # x [batch_size, seq_len, dim]

        output, hn = self.rnn(x.permute(1, 0, 2))
        hid = hn.squeeze(dim=0)  # hid [batch_size, dim]
        # TODO: the effect of pooling is not as good as hn
        # hid = torch.mean(output, dim=0) 
        out = self.lin(hid)  # out [batch_size, dim]

        # Auto Regression
        ar_out = self.ar(x[:, :, 0])

        out = out + ar_out

        return out


class RNNGATENet(nn.Module):
    def __init__(self, seq_len, hid_dim, output_dim):
        super(RNNGATENet, self).__init__()

        self.seq_len = seq_len
        self.hid_dim = hid_dim
        self.output_dim = output_dim

        self._build()

    def _build(self):
        self.rnn = RNNNet(1, self.hid_dim, self.output_dim, self.seq_len)
        self.gate = nn.Linear(self.seq_len, self.output_dim)

        # self.ar = nn.Linear(self.seq_len, self.output_dim)

    def forward(self, x):
        # x [batch_size, seq_len, dim]
         
        rnn_out = self.rnn(x[:, :, [0]])
        # gate = self.gate(x[:, :, 1])

        # out = rnn_out * gate
        # TODO: add tanh brings some optimization problem
        # out = rnn_out * torch.sigmoid(gate) * 2

        # # Auto Regression
        # ar_out = self.ar(x[:, :, 0])

        # out = out + ar_out

        # leak
        holiday = x[:, :, 1]
        gate = torch.ones_like(rnn_out)
        gate[:, 0] = gate[:, 0] * holiday[:, -5] * holiday[:, -4] * holiday[:, -3] * holiday[:, -2] * holiday[:, -1]
        gate[:, 1] = gate[:, 1] * holiday[:, -4] * holiday[:, -3] * holiday[:, -2] * holiday[:, -1]
        gate[:, 2] = gate[:, 1] * holiday[:, -3] * holiday[:, -2] * holiday[:, -1]
        gate[:, 3] = gate[:, 3] * holiday[:, -2] * holiday[:, -1]
        gate[:, 4] = gate[:, 4] * holiday[:, -1]
        # gate = torch.clamp(gate, -16, 16)
        out = rnn_out * gate

        return out
        